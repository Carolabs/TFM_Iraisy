...
class mass_yF_uS(CosimUnit):
    ...
    def evaluateSpringForces(self):
        ...
        # Las fuerzas del muelle ahora son calculadas como f = k * sign(x) * |x|^0.5
        # Coupling spring and damper
        self.__fc_sp  = -(self.__kc * np.sign(self.__x - self.__u[0]) * np.sqrt(np.abs(self.__x - self.__u[0])))
        self.__fc_dp  = -(self.__cc * (self.__xd - self.__u[1]))

        # Spring and damper betweeen mass and wall
        self.__f_sp = - self.__k * np.sign(self.__x) * np.sqrt(np.abs(self.__x))
