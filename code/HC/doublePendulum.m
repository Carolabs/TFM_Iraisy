% This file defines a class that represents a double pendulum system that
% is controlled with a hydraulic actuator.

% Input: 	force OR pressures in the cylinder
% Output: 	displacement and velocity

classdef doublePendulum < CosimUnit

    % __________________________________________________________ PROPERTIES
    properties
        Y_;         % Outputs (piston length and rate)
    end

    % ________________________________________________ PROPERTIES (PRIVATE)
    properties (Access=private)
        t;          % Internal time
        iPAR;       % Integration parameters

        th; thd; thdd;  % Position, velocity, accel. (vars. are joint angles)
        s; sd;          % Position and velocity of actuator

        uIsF;       % 1 if input is force, 0 if it is pressures
        fhyd;       % Actuation force
        p;          % Hydraulic pressures (only required if input = p)
    
        SYS;        % Physical properties
        HYD;        % Properties from hydraulic system (only if input = p)
        STORE;      % Storage
       
    end

    % _____________________________________________________________ METHODS
    methods

        % _____________________________________________________ Constructor
        function b = doublePendulum(name, inputIsF)
    
            % Call constructor of base class
            b@CosimUnit(name);

            % Select input to be force or pressures
            assert( (inputIsF==1)||(inputIsF==0),...
                "Wrong value for inputIsF");
            b.uIsF = inputIsF;

            % Set physical properties
            b.SYS = mechSetup();

            % Initial angles and rates
            b.th  = [deg2rad(30.0); deg2rad(-90.0)];
            b.thd = zeros(2,1);

            % Initial actuator displacement and velocity
            [b.s, b.sd, ~] = findSfromAngles(b.th, b.thd, b.SYS);

        end

        % ______________________________________________________ Initialize 
        function b = initialize(b,t)
            
            str = "Initializing mechanics";    disp(str);

            % Set time to externally set time
            b.t = t;
            
            % Storage
            dt = b.iPAR.dt;
            b.STORE.npoints     = round(max(b.tEnd/(dt*b.storeEvery), 1));
            b.STORE.i           = b.storeEvery-1;   % Storage counter

            np = b.STORE.npoints;
            b.STORE.t   = zeros(1, np);
            b.STORE.s   = zeros(1, np);
            b.STORE.sd  = zeros(1, np);
            b.STORE.th  = zeros(2, np);
            b.STORE.thd = zeros(2, np);
            b.STORE.thdd= zeros(2, np);
            b.STORE.storeIdx = 0;

        end

        % __________________________________________________________ doStep
        function b = doStep(b, finalT)

            % Take steps until t goes beyond finalT
            while (b.t - finalT < b.iPAR.dt/2)
                
                % Update dynamic terms
                M   = evalMassMatrix(b.th, b.SYS);
                fc  = evalVelDepForces(b.th, b.thd, b.SYS);
                fg  = evalGravForces(b.th, b.SYS);
                [b.s, b.sd, Ai] = findSfromAngles(b.th, b.thd, b.SYS);
                
                % Hydraulic force
                % If pressures were used as input, the hydraulic force
                % needs to be evaluated
                if (b.uIsF == 0)
                    b.fhyd = (b.p(2) - b.p(1))*b.HYD.A - b.HYD.c*b.sd;
                end
                fh  = Ai'*b.fhyd;

                % Evaluate accelerations
                b.thdd = M\(fg-fc + fh);

                % Store
                b.STORE.i = b.STORE.i+1;
                if b.STORE.i == b.storeEvery
                    b.STORE.storeIdx = b.STORE.storeIdx + 1;
                    si = b.STORE.storeIdx;
                    b.STORE.t(si)       = b.t;
                    b.STORE.th(:,si)    = b.th;  
                    b.STORE.thd(:,si)   = b.thd; 
                    b.STORE.thdd(:,si)  = b.thdd;
                    b.STORE.s(si)       = b.s;  
                    b.STORE.sd(si)      = b.sd;
                    b.STORE.F(si)       = b.fhyd;
                    b.STORE.i = 0;
                end

                % Integrate velocities
                b.thd  = b.thd + b.iPAR.dt*b.thdd;

                % Integrate positions
                b.th   = b.th + b.iPAR.dt*b.thd;
 
                % Increase time
                b.t   = b.t + b.iPAR.dt;
            end
        end

        % _______________________________________________________ Terminate 
        function b = terminate(b, t)
            str = "Terminating mechanics.";    disp(str);
        end

        % ____________________________________________________ Read outputs
        function y = readOutputs(b)

            % Re-evaluate displacements
            [b.s, b.sd, ~] = findSfromAngles(b.th, b.thd, b.SYS);
            b.Y_ = [b.s;b.sd];

            y = b.Y_;
        end
        
        % ______________________________________________________ Set inputs
        function setInputs(b, u)
            
            if (b.uIsF == 1)
                % Hydraulic force
                b.fhyd     = u;
            elseif (b.uIsF == 0)
                % Pressures
                b.p         = [u(1);u(2)];
                b.HYD.A     = u(3); % Piston area and damping
                b.HYD.c     = u(4);
            end
        end

        % ________________________________________ Evaluate effective force
        function feff0 = evalEffectiveForce(b)
            % Returns the effective force of the mechanical system 
            % along the direction of the hydraulic actuator

            % In direct co-simulation, this function is used only 
            % to initialize the hydraulics
            MM          = evalMassMatrix(b.th, b.SYS);
            fg          = evalGravForces(b.th, b.SYS);
            fc          = evalVelDepForces(b.th, b.thd, b.SYS);

            % Eval Jacobian matrix of cylinder direction
            [~, ~, J]   = findSfromAngles(b.th, b.thd, b.SYS);

            meff        = (J*MM^(-1)*J')^(-1);
            feff0       = meff * J * MM^(-1) * (fg-fc);

        end
        
        % _______________________________________________________ Accessors
        function STORE = getSTORE(b); STORE = b.STORE; end
        function setStepSize(b, dt); b.iPAR.dt = dt; end

    end

end