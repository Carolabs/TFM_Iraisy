...
\commentFG{Una cosa: si el párrafo que va después de la ecuación no está unido a ella en el código te va a quedar una indentación al principio, que no queda muy bien en el texto. Para evitar eso, es necesario que no haya líneas en blanco entre el texto y la ecuación. Yo suelo resolverlo poniendo un comentario -mira la diferencia entre (7.1) y (7.2).}
...
Otro punto a tener en cuenta, es que es necesario limitar la corrección que aplica el estimador, es decir, la diferencia entre la fuerza devuelta por el subsistema con direct feedthrough y la salida del estimador.
